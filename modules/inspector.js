...
import * as decorTool from './decorTool.js';

let onPropertyChange = null;
let onRequestVarInfo = null;
let onRequestDeclared = null;
...
let isLite = () => false;
let tools = [];   // кнопки инструментов под шапкой: топ-бар, облегчение, код…
let onQuickTarget = null;   // выбрать элемент без прицела
const LITE_SHADOW_BLUR = 10;
...
/* ============================================================
   ГЕОМЕТРИЯ И МАСШТАБ ПАНЕЛИ

   Раньше панель была жёстко 340 px шириной, без изменения размера
   и без настройки шрифта. Теперь размер, положение и масштаб
   запоминаются между сессиями.

   Масштаб реализован через zoom на содержимом: в стилях панели
   десятки жёстко заданных размеров в px, и переопределять каждый
   ради одного ползунка не имеет смысла.
============================================================ */
const UI_KEY = 'vte-inspector-ui';
const ZOOM_MIN = 0.75;
const ZOOM_MAX = 1.6;
...
function loadUi() {
    ...
    const z = Number(ui.zoom);
    ui.zoom = Number.isFinite(z) ? Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, z)) : 1;
}
...
function applyZoom() {
    if (!panel) return;
    panel.style.setProperty('--vte-zoom', String(ui.zoom));
    const label = panel.querySelector('#vte-zoom-val');
    if (label) label.textContent = `${Math.round(ui.zoom * 100)}%`;
}

function setZoom(next, quiet) {
    const v = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, Math.round(next * 100) / 100));
    if (v === ui.zoom) return;
    ui.zoom = v;
    applyZoom();
    saveUi();
    if (!quiet) flash(`Масштаб ${Math.round(v * 100)}%`);
}
...
function applyGeometry() {
    ...
    if (ui.left != null && ui.top != null) {
        ...
        panel.style.top = `${Math.min(Math.max(0, ui.top), window.innerHeight - 60)}px`;
    }
}
...
/** Ctrl + колесо над панелью меняет масштаб */
function onWheelZoom(e) {
    if (!(e.ctrlKey || e.metaKey)) return;
    e.preventDefault();
    setZoom(ui.zoom + (e.deltaY < 0 ? ZOOM_STEP : -ZOOM_STEP), true);
}

/** Ручка изменения размера в правом нижнем углу */
function makeResizable(box) {
    const grip = h('div.vte-panel-grip', {
    ...
    grip.addEventListener('pointerdown', (e) => {
        ...
        // Панель прижата к правому краю: без явного left она будет
        // растягиваться в обратную сторону
...
/* ============================================================
   СОСТОЯНИЕ ПАНЕЛИ МЕЖДУ ПЕРЕРИСОВКАМИ

   refresh() раньше пересобирал все четыре вкладки целиком на каждый
   коммит: около 120 узлов с обработчиками заново. Из-за этого слетала
   прокрутка, раскрывались свёрнутые секции, терялся фокус и стирался
   недописанный текст в полях.

   Теперь: рисуем только активную вкладку, остальные помечаем «грязными»
   и обновляем при переключении. Свёрнутость и прокрутка запоминаются,
   а пока курсор стоит в поле ввода — перерисовка откладывается.
============================================================ */
let activeTab = 'colors';
const dirtyPanes = new Set();
const sectionOpen = new Map();   // название секции -> раскрыта ли
let paneScroll = 0;
let pendingRefresh = false;

/** Курсор внутри поля панели: перерисовка стёрла бы набранное */
function typingInPanel() {
    const a = document.activeElement;
    if (!a || !panel || !panel.contains(a)) return false;
    const tag = a.tagName;
    if (tag === 'TEXTAREA') return true;
    if (tag !== 'INPUT') return false;
    return !['range', 'checkbox', 'radio', 'color', 'button'].includes(a.type);
}
...
export function init(options = {}) {
    ...
    onRequestDeclared = options.onRequestDeclared || (() => null);
    ...
    if (typeof options.isLite === 'function') isLite = options.isLite;
    tools = Array.isArray(options.tools) ? options.tools : [];
    onQuickTarget = options.onQuickTarget || null;
    ...
    decorTool.init({
        ...
        // Живой предпросмотр: декор в CSS ещё не записан
        onLive: (decls) => {
            if (!sel) return;
            for (const [p, v] of Object.entries(decls)) {
                if (v === '') continue;
                onPropertyChange(sel, p, v, { useVariables: false, live: true });
            }
        },
        ...
        onApply: (decls, info) => {
            ...
            // decls === null означает «поправить только хозяина»
            if (decls) writeDecls(sel, { ...decls });
            ...
            // Мобильные значения идут в @media отдельной пачкой
            if (info?.mediaDecls && onBatchChange) {
                for (const [media, md] of Object.entries(info.mediaDecls)) {
                    onBatchChange(sel, md, { media });
                }
            }
...
export function createPanel() {
    ...
    els.zoomBox = h('div.vte-zoom-box', {}, [
        iconBtn('fa-magnifying-glass-minus', 'Мельче (Ctrl + колесо)',
            () => setZoom(ui.zoom - ZOOM_STEP)),
        h('span#vte-zoom-val.vte-zoom-val', {
            title: 'Сбросить масштаб',
            text: '100%',
            on: { click: () => setZoom(1) },
        }),
        iconBtn('fa-magnifying-glass-plus', 'Крупнее (Ctrl + колесо)',
            () => setZoom(ui.zoom + ZOOM_STEP)),
    ]);
    ...
    const tabDefs = [
        ...
        ['decor', 'Декор', 'fa-panorama'],
        ['type', 'Текст', 'fa-font'],
        ['fx', 'Эффекты', 'fa-wand-sparkles'],
        // «Шрифты» переехали в строку инструментов: они не про один элемент
    ];

    els.tabs = h('div.vte-tabs');
    els.panes = {};
    const paneWrap = h('div.vte-panes');
    els.paneWrap = paneWrap;
    ...
    // Без прицела: быстрые цели — часто нужные части интерфейса одним кликом
    const QUICK = [
        ['Вся страница', 'body'],
        ['Чат', '#chat'],
        ['Сообщение', '.mes'],
        ['Текст сообщений', '.mes_text'],
        ['Имена', '.name_text'],
        ['Нижняя панель', '#send_form'],
        ['Поле ввода', '#send_textarea'],
        ['Топ-бар', '#top-bar'],
        ['Панели меню', '.drawer-content'],
    ];
    els.empty = h('div#vte-empty.vte-empty', {}, [
        h('span.vte-empty-ic', {}, [icon('fa-hand-pointer')]),
        h('div', { text: 'Элемент не выбран' }),
        h('small', { text: 'Кликните прицелом по интерфейсу или выберите сразу:' }),
        h('div.vte-quick', {}, QUICK.map(([label, q]) => h('button.vte-quick-btn', {
            type: 'button',
            title: q,
            on: {
                click: () => {
                    const el = document.querySelector(q);
                    if (el) onQuickTarget?.(el);
                },
            },
        }, [h('span', { text: label })]))),
    ]);
    ...
    // Инструменты — всегда под шапкой, даже когда элемент не выбран
    els.tools = h('div.vte-tools-row', {}, tools.map(t => h('button.vte-tools-btn', {
        type: 'button',
        title: t.title || t.label,
        dataset: { tool: t.id },
        on: { click: (e) => { t.onClick?.(e.currentTarget); refreshTools(); } },
    }, [icon(t.icon), h('span', { text: ` ${t.label}` })])));

    panel = h('div#vte-inspector-panel.vte-panel', {}, [
        els.header, tools.length ? els.tools : null, els.empty, els.content, els.status, els.footer,
    ].filter(Boolean));
    ...
    panel.addEventListener('wheel', onWheelZoom, { passive: false });
...
function switchTab(id) {
    ...
    // Вкладка могла устареть, пока была скрыта — обновляем при показе
    if (dirtyPanes.has(id) && el) {
        cs = rules.computedFor(el, pseudo);
        renderPane(id);
    }

    if (els.paneWrap) els.paneWrap.scrollTop = 0;
    ...
    if (id === 'decor') {
        // Декор всегда живёт в слое: на самом элементе он затёр бы его фон
        if (!pseudo) setPseudo('::after');
        if (!decorMounted) {
            decorMounted = true;
            decorTool.mount(els.panes.decor);
        }
        decorTool.update({ selector: sel, pseudo, computed: cs, element: el });
    }
}
...
export function populateProperties(element, computed, selector, info) {
    ...
    // Сменилась цель — устарело всё
    dirtyPanes.add('colors');
    dirtyPanes.add('layout');
    dirtyPanes.add('type');
    dirtyPanes.add('fx');
    paneScroll = 0;
...
/** Перерисовать одну вкладку и снять с неё метку «грязная» */
function renderPane(id) {
    switch (id) {
        case 'colors': renderColors(); break;
        case 'layout': renderLayout(); break;
        case 'type':   renderType(); break;
        case 'fx':     renderFx(); break;
        default: return;   // fonts и icon живут своей жизнью
    }
...
function refresh() {
    ...
    // Человек печатает — перерисовка затрёт набранное. Ждём, пока уйдёт фокус.
    if (typingInPanel()) {
        if (!pendingRefresh) {
            pendingRefresh = true;
            const retry = () => {
                ...
                setTimeout(() => { if (!typingInPanel()) refresh(); }, 60);
            };
    ...
    // Прокрутку снимаем до перестройки и возвращаем после: иначе список
    // свойств прыгал в начало при каждой правке
    const wrap = els.paneWrap;
    if (wrap) paneScroll = wrap.scrollTop;

    for (const id of ['colors', 'layout', 'type', 'fx']) {
        if (id !== activeTab) dirtyPanes.add(id);
    }
    renderPane(activeTab);

    if (wrap) {
        wrap.scrollTop = paneScroll;
        requestAnimationFrame(() => { wrap.scrollTop = paneScroll; });
    }

    if (iconMounted) iconTool.update({ selector: sel, pseudo, computed: cs, element: el });
    if (decorMounted && activeTab === 'decor') {
        decorTool.update({ selector: sel, pseudo, computed: cs, element: el });
    }
    showVarHint('background-color');
}
...
/**
 * Отметка «клики сквозь». Такой элемент выбран намеренно, но без подписи
 * непонятно, почему прицел взял слой, по которому нельзя щёлкнуть.
 */
function renderDecorNote() {
    const old = els.content.querySelector('#vte-decor-note');
    if (old) old.remove();
    if (!el) return;

    let through = false;
    try { through = getComputedStyle(el).pointerEvents === 'none'; } catch {}
    if (!through) return;

    const note = h('div#vte-decor-note.vte-icon-stash', {
        style: 'display:flex; margin:0; border-radius:0; border-left:0; border-right:0',
    }, [
        h('span.vte-icon-stash-ic', {}, [icon('fa-hand')]),
        h('span.vte-icon-stash-text', {
            text: 'Декоративный слой: pointer-events: none, клики проходят сквозь него. '
                + 'Стилизовать можно как обычно. Ctrl в режиме выбора — брать то, '
                + 'что лежит под ним.',
        }),
    ]);

    els.selectorRow.insertAdjacentElement('beforebegin', note);
}
...
function colorField(label, prop, value, allowGradient) {
    ...
    hexInput.addEventListener('change', () => {
        ...
        // Прозрачный фон (у большинства элементов ST он такой) даёт ползунок
        // прозрачности на нуле. Раньше введённый цвет получал ту же нулевую
        // прозрачность и был невидим: «ввела цвет — ничего не поменялось».
        // Если в самом коде прозрачности нет, а ползунок на нуле — берём 100%.
        const typedAlpha = /^#?(?:[0-9a-f]{4}|[0-9a-f]{8})$/i.test(raw);
        const a = Number(alphaRange.value) / 100;
        push(withAlpha(norm, !typedAlpha && a === 0 ? 1 : a), false);
    });
...
function iconSizeRow() {
    ...
    const apply = (raw, live) => {
        ...
        // min-width и min-height, равные width и height, ничего не держат.
        // Пишем их пустыми, чтобы убрать наследие прошлых версий.
        const decls = {
            'width': `${n}px`,
            'height': `${n}px`,
            'min-width': '',
...
/**
 * Причины, по которым transform не даст видимого результата.
 *
 * По спецификации transform не применяется к строчным элементам вообще —
 * браузер просто выбрасывает свойство, никакой ошибки при этом не видно.
 * Панель раньше писала transform в любую цель и молчала, поэтому выглядело
 * это как «расширение не работает».
 *
 * Вторая причина — обрезка у предка: overflow: hidden или contain: paint
 * отрежут всё, что выехало за его границы, даже если transform применился.
 */
function transformBlockers() {
    const out = { inline: false, clipper: null };
    if (!cs) return out;

    const disp = String(cs.display || '');
    out.inline = disp === 'inline';
    ...
    while (p && depth < 6 && p !== document.body) {
        try {
            ...
            const clipped = /hidden|clip/.test(pcs.overflow || '')
                || /paint|content|strict/.test(pcs.contain || '');
            if (clipped) {
                out.clipper = p.id ? '#' + p.id
...
function transformRow() {
    const state = parseTransform(declaredOr('transform', cs.transform));
    const block = transformBlockers();
    ...
    const build = () => {
        ...
        // Незнакомые функции возвращаем на место: translateZ(0), perspective(),
        // skew() и прочее, что панель не редактирует, но и терять не должна
        if (state.rest) parts.push(state.rest);
        return parts.length ? parts.join(' ') : 'none';
    };

    const fixInline = block.inline
        ? h('button.vte-btn', {
            type: 'button',
            title: 'Дописать display: inline-block, без него transform не действует',
            on: {
                click: () => {
                    writeDecls(sel, { 'display': 'inline-block' });
                    flash('Добавлено display: inline-block');
                    setTimeout(refresh, 340);
                },
            },
        }, [icon('fa-wrench'), h('span', { text: ' Исправить display' })])
        : null;

    return h('div.vte-subgroup', {}, [
        block.inline
            ? h('small.vte-note', {
                style: 'color: var(--vte-warn)',
                text: 'Цель строчная (display: inline) — transform к ней не применяется '
                    + 'вообще. Сначала задайте inline-block или block.',
              })
            : null,
        fixInline,
        block.clipper
            ? h('small.vte-note', {
                text: `Предок ${block.clipper} обрезает содержимое `
                    + '(overflow: hidden или contain: paint). Сдвиг за его границы '
                    + 'будет виден только внутри этой области.',
              })
            : null,
    ...
    pane.append(
        ...
        section('Прозрачность и фильтры', [
            sliderRow('Прозрачность', 'opacity', Math.round((parseFloat(cs.opacity) || 1) * 100),
                0, 100, 1, '%', 'ratio'),
            filterGroup(),
            // Размытие фона — самый тяжёлый эффект: пересчитывается на каждом
            // кадре прокрутки. В лёгком режиме показываем только чтобы убрать.
            (!isLite() || backdropBlurValue() > 0)
                ? sliderRow(isLite() ? 'Размытие фона (тяжёлое)' : 'Размытие фона',
                    'backdrop-filter', backdropBlurValue(), 0, isLite() ? backdropBlurValue() : 40, 1, 'px', 'blur')
                : null,
            isLite() && backdropBlurValue() > 0
                ? h('small.vte-note', { text: 'Размытие фона сильно грузит интерфейс. Сдвиньте в 0, чтобы убрать.' })
                : null,
        ].filter(Boolean)),
...
/**
 * Раздел «Градиент фона». В лёгком режиме градиент не предлагаем:
 * если у элемента его нет — раздела нет вовсе, если есть — кнопка
 * «Заменить сплошным цветом».
 */
function gradientSection(toIconTab) {
    const current = /gradient/i.test(cs.backgroundImage || '') ? cs.backgroundImage : '';

    if (!isLite()) {
        return section('Градиент фона', [
            colorField('Градиент', 'background-image', current, true),
            h('small.vte-note', {
    ...
    const first = (current.match(/#[0-9a-f]{3,8}\b|rgba?\([^()]*\)|hsla?\([^()]*\)/i) || [])[0] || '';
    return section('Градиент фона', [
        h('small.vte-note', {
            text: 'У элемента градиентный фон. Градиенты перерисовываются при каждом изменении '
                + 'размера и заметно грузят интерфейс.',
        }),
        h('button.vte-btn', {
            type: 'button',
            title: first ? `Фон станет ${first}` : 'Градиент будет убран',
            on: {
                click: () => {
                    // Одной пачкой и без «none → убрать строку»: градиент может
                    // идти из стилей самой ST, и тогда 'none' обязателен
                    const decls = { 'background-image': 'none' };
                    // Без градиента фон мог стать прозрачным — подставляем
                    // цвет первой точки, если своего сплошного цвета нет
                    const bg = parseRgba(cs.backgroundColor);
                    if (first && (!bg || bg.a === 0)) decls['background-color'] = first;
                    writeDecls(sel, decls);
                },
            },
        }, [h('i', { className: 'fa-solid fa-fill' }), h('span', { text: ' Заменить сплошным цветом' })]),
    ]);
}

function filterGroup() {
    const state = parseFilter(declaredOr('filter', cs.filter));
    ...
    const build = () => {
        ...
        // drop-shadow, invert, sepia и прочее, что ползунков не имеет
        if (state.rest) parts.push(state.rest);
        return parts.length ? parts.join(' ') : 'none';
    };

    return h('div.vte-subgroup', {}, [
        // blur() в filter тоже тяжёлый. В лёгком режиме — только чтобы убрать
        (!isLite() || state.blur > 0)
            ? groupSlider('Размытие', state, 'blur', 0, isLite() ? state.blur : 30, 'px', 'filter', build)
            : null,
...
function shadowGroup() {
    const state = parseShadow(declaredOr('box-shadow', cs.boxShadow));

    const build = () => {
        const inset = state.inset ? 'inset ' : '';
        const first = `${inset}${state.x}px ${state.y}px ${state.blur}px ${state.spread}px ${state.color}`;
        // Остальные тени списка возвращаем как были: правится только первая
        return state.rest ? `${first}, ${state.rest}` : first;
    };
    ...
    return h('div.vte-subgroup', {}, [
        state.rest
            ? h('small.vte-note', {
                text: 'У элемента несколько теней. Правится первая, '
                    + 'остальные сохраняются как есть.',
              })
            : null,
        h('div.vte-field', {}, [
        ...
        // Мягкая тень с большим размытием на крупном блоке заметно грузит
        // прокрутку. В лёгком режиме потолок 10px (или текущее значение,
        // если оно уже больше — чтобы его можно было уменьшить)
        groupSlider('Размытие', state, 'blur', 0,
            isLite() ? Math.max(LITE_SHADOW_BLUR, state.blur) : 80, 'px', 'box-shadow', build),
...
function section(title, children) {
    // По умолчанию раскрыта. Состояние живёт в модуле, а не в DOM,
    // поэтому переживает перерисовку вкладки.
    const wasOpen = sectionOpen.has(title) ? sectionOpen.get(title) : true;

    const body = h('div.vte-section-body', {}, children);
    body.style.display = wasOpen ? 'flex' : 'none';

    const chev = h('span.vte-section-chev', {}, [icon('fa-chevron-down')]);
    chev.classList.toggle('vte-rotated', !wasOpen);
    ...
    const head = h('button.vte-section-head', {
        ...
        on: {
            click: () => {
                ...
                sectionOpen.set(title, !open);
            },
        },
    }, [h('span', { text: title }), chev]);

    return h('div.vte-section', { dataset: { sec: title } }, [head, body]);
}
...
function writeDecls(target, decls, opts) {
    if (!target) return;
    if (onBatchChange) { onBatchChange(target, decls, opts); return; }
    for (const [p, v] of Object.entries(decls)) {
...
function commitNow(property, value) {
    if (!sel) return;
    const prev = timers.get(property);
    if (prev) clearTimeout(prev.t);

    const target = sel;
    const fire = () => {
        timers.delete(property);
        onPropertyChange(target, property, value, { useVariables: useVars });
        showVarHint(property);
    };
    timers.set(property, { t: setTimeout(fire, DEBOUNCE), fire });
}

/**
 * Выполнить отложенные записи прямо сейчас.
 *
 * Нужно перед отменой. Сценарий: ввели число в поле и сразу нажали
 * «Отменить». Уход фокуса с поля запускает запись с задержкой, отмена
 * успевает раньше, а запись срабатывает уже после неё и возвращает
 * всё как было. Выглядело как «отмена не сработала».
 */
export function flushPending() {
    if (!timers.size) return false;
    const list = [...timers.values()];
    timers.clear();
    for (const { t, fire } of list) {
        clearTimeout(t);
        try { fire(); } catch {}
    }
    return true;
...
function showVarHint(property) {
    ...
    els.varHint.append(icon('fa-plus'), h('span', { text: ' Новое правило в «Мои правки»' }));
}
...
/**
 * Заявленное значение из CSS, если удалось получить, иначе вычисленное.
 *
 * Заявленный текст сохраняет то, что браузер нормализует и тем самым теряет:
 * translateZ(0), drop-shadow(), список из нескольких теней.
 */
function declaredOr(property, computedValue) {
    try {
        const declared = onRequestDeclared(sel, property);
        if (declared != null && String(declared).trim()) return String(declared);
    } catch {}
    return computedValue;
}

/** Делит по запятым верхнего уровня: 'a(1,2), b(3)' → ['a(1,2)', 'b(3)'] */
...
/**
 * Разбор filter с сохранением незнакомых функций.
 *
 * Раньше всё, кроме шести известных, отбрасывалось: первое движение любого
 * ползунка стирало drop-shadow, invert, sepia и opacity из фильтра.
 * Теперь незнакомое складывается в state.rest и приклеивается обратно.
 */
function parseFilter(value) {
    const state = {
        blur: 0, brightness: 100, saturate: 100,
        contrast: 100, hue: 0, grayscale: 0, rest: '',
    };
    const raw = String(value ?? '').trim();
    if (!raw || raw === 'none') return state;

    const pct = (n, isPct) => Math.round(isPct ? n : n * 100);
    const rest = [];
    const fnRe = /([a-zA-Z-]+)\(([^()]*(?:\([^()]*\)[^()]*)*)\)/g;
    let m;
    ...
    while ((m = fnRe.exec(raw)) !== null) {
        ...
        const isPct = numMatch ? numMatch[2] === '%' : false;

        switch (name) {
            case 'blur':        state.blur = Number.isFinite(n) ? n : 0; break;
            case 'brightness':  state.brightness = Number.isFinite(n) ? pct(n, isPct) : 100; break;
            case 'saturate':    state.saturate = Number.isFinite(n) ? pct(n, isPct) : 100; break;
            case 'contrast':    state.contrast = Number.isFinite(n) ? pct(n, isPct) : 100; break;
            case 'grayscale':   state.grayscale = Number.isFinite(n) ? pct(n, isPct) : 0; break;
            case 'hue-rotate':  state.hue = Number.isFinite(n) ? n : 0; break;
            default:            rest.push(m[0]);
        }
    }

    state.rest = rest.join(' ');
    return state;
}

/**
 * Разбор box-shadow. Редактируется ПЕРВАЯ тень, остальные сохраняются
 * в state.rest без изменений.
 *
 * Раньше список теней читался как одна: 'box-shadow: A, B' после правки
 * ползунка превращался в одну тень, вторая исчезала безвозвратно.
 */
function parseShadow(value) {
    const state = {
        x: 0, y: 4, blur: 12, spread: 0,
        color: 'rgba(0, 0, 0, 0.5)', inset: false, rest: '',
    };
    const raw = String(value ?? '').trim();
    if (!raw || raw === 'none') return state;

    const list = splitTopLevel(raw);
    const first = list[0] || '';
    state.rest = list.slice(1).join(', ');

    state.inset = /\binset\b/i.test(first);

    const colorMatch = first.match(
        /(rgba?\([^)]+\)|hsla?\([^)]+\)|#[0-9a-f]{3,8}|\b[a-z]{3,20}\b(?!\s*\())/i);
    if (colorMatch && !/^(inset|px|em|rem)$/i.test(colorMatch[1])) {
        state.color = colorMatch[1];
    }

    const nums = first
        .replace(/(rgba?\([^)]+\)|hsla?\([^)]+\)|#[0-9a-f]{3,8}|inset)/gi, '')
        .match(/-?[\d.]+(?:px|em|rem)?/g);
    if (nums) {
...
/**
 * Разбор transform с сохранением незнакомых функций.
 *
 * Главный случай: 'transform: translateZ(0)' — приём для включения
 * аппаратного слоя. Он не переводится в translate/scale/rotate, поэтому
 * старый парсер его терял, и первое же движение ползунка отключало
 * аппаратное ускорение у элемента. Теперь такие функции идут в state.rest.
 */
function parseTransform(value) {
    const out = { x: 0, y: 0, scale: 100, rotate: 0, rest: '' };
    const raw = String(value ?? '').trim();
    if (!raw || raw === 'none') return out;

    // Матрица приходит из getComputedStyle. Разобрать её на осмысленные
    // части можно только приблизительно, зато без потери геометрии.
    const mtx = raw.match(/^matrix(3d)?\(([^)]+)\)$/i);
    if (mtx) {
        const p = mtx[2].split(',').map(Number);
        if (mtx[1]) {
            out.x = Math.round(p[12] || 0);
            out.y = Math.round(p[13] || 0);
    ...
    while ((m = fnRe.exec(raw)) !== null) {
        ...
        switch (name) {
            ...
            case 'scale':
                out.scale = Math.round((parseFloat(args[0]) || 1) * 100);
                break;
            case 'rotate':
                out.rotate = Math.round(parseFloat(args[0]) || 0);
                break;
            default:
                rest.push(m[0]);
        }
...
/**
 * Разбор цвета для полей панели.
 *
 * Основную работу делает пикер: его парсер знает именованные цвета и
 * современные форматы, потому что спрашивает браузер. Здесь только быстрый
 * путь для двух самых частых записей — их панель получает от
 * getComputedStyle сотнями.
 */
function parseRgba(value) {
    const v = String(value ?? '').trim();
    if (!v || /gradient|url\(|^var\(/i.test(v)) return null;

    let m = v.match(/^#([0-9a-f]{3,8})$/i);
    if (m) {
        const len = m[1].length;
        // Только 3, 4, 6 и 8 знаков валидны. Прежний код из '#12345'
        // вычитывал мусор и выдавал его за цвет.
        if (![3, 4, 6, 8].includes(len)) return null;
        let hex = m[1];
        if (len === 3 || len === 4) hex = hex.split('').map(c => c + c).join('');
        return {
    ...
    if (m) {
        ...
        return {
            r: Math.round(Math.max(0, Math.min(255, p[0]))),
            g: Math.round(Math.max(0, Math.min(255, p[1]))),
            b: Math.round(Math.max(0, Math.min(255, p[2]))),
            a: p[3] == null || !Number.isFinite(p[3])
                ? 1 : Math.max(0, Math.min(1, p[3])),
        };
    }

    // Всё остальное — hsl, oklch, именованные цвета — умеет пикер
    try {
        const p = picker?.utils?.toRgbaParts?.(v);
        if (p) return p;
    } catch {}
