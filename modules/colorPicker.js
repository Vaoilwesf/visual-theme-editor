...
export function init(options = {}) {
    // build() при инициализации не вызываем: попап собирался при загрузке ST
    // и висел в DOM, даже если редактор ни разу не открывали.
    // Он всё равно создаётся при первом open().
    setSwatches(options.swatches || []);
}
...
export function setSwatches(list) {
    ...
    for (const s of src) {
        if (!s || !isColorLike(s.value)) continue;
        // Одинаковые цвета под разными именами дают ряд неразличимых
        // плашек. Оставляем первое имя, остальные пишем в подсказку.
        const key = normalizeColor(s.value);
        if (!key) continue;
        const prev = seen.has(key) ? out.find(o => o.key === key) : null;
        if (prev) {
            prev.alias.push(s.name);
            continue;
        }
        seen.add(key);
        out.push({ name: s.name, value: s.value, key, alias: [] });
    }
...
function renderSwatches() {
    ...
    for (const s of list) {
        const tip = s.alias.length
            ? `${s.name}\n${s.value}\nто же значение: ${s.alias.join(', ')}`
            : `${s.name}\n${s.value}`;
...
function applyHexInput() {
    ...
    // Человек мог напечатать 'f4db9a' без решётки — это ходовой случай.
    // Раньше такое молча отбрасывалось.
    const candidates = /^[0-9a-f]{3,8}$/i.test(raw) ? ['#' + raw, raw] : [raw];

    let parsed = null;
    for (const c of candidates) {
        parsed = normalizeColor(c);
        if (parsed) break;
    }

    if (!parsed) {
        // Показываем, что значение не принято, и возвращаем прежнее
        els.hex.classList.add('vte-cp-bad');
        setTimeout(() => els.hex.classList.remove('vte-cp-bad'), 900);
        syncUI();
        return;
    }
...
/* ---------- Проверка через браузер ----------
   Присваивание невалидного значения в style не бросает ошибку: оно молча
   игнорируется, и свойство остаётся пустым. Именно на этом строится
   проверка. Прежний код вместо этого читал getComputedStyle и сравнивал
   результат с исходной строкой — для 'solid' или '1.04' возвращался цвет,
   унаследованный от body, и любая нецветовая переменная темы считалась
   цветом. В палитру пикера из-за этого лезли отступы и множители.

   Элемент в документ не вставляется и computed-стиль не читается:
   и то, и другое заставляет браузер пересчитать раскладку, а проверка
   вызывается на каждую переменную темы. */
const probeEl = document.createElement('span');
const parseCache = new Map();

/** Значение принято как цвет? Возвращает нормализованный вид или null */
function browserColor(value) {
    ...
    try {
        ...
        // Пустая строка означает, что браузер значение отверг
        out = probeEl.style.color || null;
    } catch {}

    if (parseCache.size > 400) parseCache.clear();
    parseCache.set(value, out);
    return out;
}
...
function toRgbaParts(value) {
    ...
    // Составные значения цветом не являются, даже если содержат цвет внутри:
    // 'var(--x)' без контекста не разобрать, 'gradient' и 'url' — не цвет
    if (/^var\(/i.test(v) || /gradient|url\(/i.test(v)) return null;

    let m = v.match(/^#([0-9a-f]{3,8})$/i);
    if (m) {
        const len = m[1].length;
        // Валидны только 3, 4, 6 и 8 знаков. '#12345' браузер отвергает,
        // а прежний код читал из него мусор.
    ...
    if (m) {
        ...
        // Проценты в rgb() допустимы: rgb(100% 0% 0%)
        const num = (s, scale) => {
            const n = parseFloat(s);
            if (!Number.isFinite(n)) return null;
            return /%$/.test(s) ? (n / 100) * scale : n;
        };
        const r = num(raw[0], 255);
        const g = num(raw[1], 255);
        const b = num(raw[2], 255);
        if (r == null || g == null || b == null) return null;
        const a = raw[3] == null ? 1 : (num(raw[3], 1) ?? 1);
        return {
    ...
    if (m) {
        const raw = m[1].split(/[,\s/]+/).filter(Boolean);
        if (raw.length < 3) return null;
        const hue = parseFloat(raw[0]);
        const sat = parseFloat(raw[1]);
        const lig = parseFloat(raw[2]);
        if (![hue, sat, lig].every(Number.isFinite)) return null;
        const rgb = hslToRgb(hue, sat / 100, lig / 100);
        let a = 1;
        if (raw[3] != null) {
            const av = parseFloat(raw[3]);
            if (Number.isFinite(av)) a = /%$/.test(raw[3]) ? av / 100 : av;
        }
        return { ...rgb, a: Math.max(0, Math.min(1, a)) };
    }

    // Именованные цвета и современные форматы — спрашиваем браузер.
    // Он вернёт rgb() или rgba(), которые уже разобраны выше.
    const accepted = browserColor(v);
    if (!accepted) return null;
    // Защита от бесконечности: если браузер отдал то же самое,
    // разбирать заново нечего
    if (accepted.trim().toLowerCase() === v.toLowerCase()) return null;
    return toRgbaParts(accepted);
}
...
/**
 * Годится ли значение переменной для плашки в палитре.
 *
 * Строже, чем toRgbaParts: сюда попадают переменные темы, и полностью
 * прозрачное значение показывать бессмысленно — плашка будет пустой,
 * человек решит, что палитра сломалась.
 */
function isColorLike(v) {
    const s = String(v ?? '').trim();
    if (!s) return false;

    // Ключевые слова, которые формально цвета, но в палитре только мешают
    if (/^(transparent|currentcolor|inherit|initial|unset|revert|none|auto)$/i.test(s)) {
        return false;
    }

    const p = toRgbaParts(s);
    if (!p) return false;
    return p.a > 0.02;
}
