...
export function setHighlightColor(color) {
    ...
    // Без свечения: рамка ездит за курсором, и мягкая тень перерисовывалась
    // на каждом движении мыши
    box.style.boxShadow = `0 0 0 1px ${withAlpha(color, 0.35)}`;
...
/**
 * Обработчик мыши только запоминает событие. Тяжёлая часть — поиск цели
 * и отрисовка рамок — выполняется один раз за кадр.
 *
 * Раньше draw() шёл на каждое событие mousemove, а внутри четыре
 * getBoundingClientRect и getComputedStyle подряд. Каждый из них заставляет
 * браузер пересчитать раскладку, поэтому прицел заметно тормозил.
 */
function onMove(e) {
    if (!active) return;
    lastEvent = e;
    ...
    moveRaf = requestAnimationFrame(() => {
        moveRaf = null;
        if (active && lastEvent) processMove(lastEvent);
    });
}
...
// Насколько декоративный слой должен быть меньше кликабельного, чтобы
// перебить его. 0.7 подобрано так, чтобы мелкие бейджи и иконки выигрывали,
// а подложки во весь блок — нет.
const DECOR_AREA_RATIO = 0.7;

// Ctrl зажат — берём строго то, что поймал бы обычный клик.
// Ручной обход на случай, когда эвристика площади не угадала.
let ctrlHeld = false;

/**
 * Цель под точкой.
 *
 * pointer-events: none больше не значит «пропустить». Такие элементы —
 * подсказки, бейджи, декоративные слои — как раз и надо уметь стилизовать,
 * а раньше вместо них выбиралось то, что лежит под ними.
 *
 * Совсем убрать проверку тоже нельзя: широкая полупрозрачная подложка во
 * весь блок начала бы перехватывать прицел вместо иконки. Поэтому сравниваем
 * площади — декоративный слой побеждает только если заметно меньше того
 * кликабельного элемента, что под ним.
 */
function exactAt(x, y) {
    ...
    // stackAt отдаёт слои сверху вниз, поэтому всё, что встретилось до
    // первого кликабельного, лежит НАД ним
    for (const node of stack) {
        if (isClickThrough(node)) {
            if (isPainted(node)) decor.push(node);
            continue;
        }
        base = node;
        break;
    ...
    if (!decor.length) return base;

    let best = null;
    let bestArea = Infinity;
    for (const node of decor) {
        const a = areaOf(node);
        if (a > 0 && a < bestArea) { bestArea = a; best = node; }
    }

    if (!best) return base;
    if (!base) return best;   // кликабельного под точкой вообще нет

    return bestArea < areaOf(base) * DECOR_AREA_RATIO ? best : base;
}
...
/**
 * Элемент реально что-то рисует на экране.
 *
 * Нужно, чтобы в кандидаты не попадали пустые обёртки и распорки с
 * pointer-events: none — они ничего не показывают, и править их бессмысленно.
 */
function isPainted(node) {
    try {
        ...
        const hasBorder = ['borderTopWidth', 'borderRightWidth',
        ...
        const hasOutline = (parseFloat(cs.outlineWidth) || 0) > 0
            && cs.outlineStyle !== 'none';

        const hasText = !!String(node.textContent || '').trim();

        // Псевдоэлементы: у бейджа или иконки вся картинка может быть в ::before
        let hasPseudo = false;
        for (const p of ['::before', '::after']) {
            const pcs = getComputedStyle(node, p);
            if (!pcs) continue;
            const c = pcs.content;
            if (c && c !== 'none' && c !== 'normal') { hasPseudo = true; break; }
        }

        return hasBg || hasBorder || hasMask || hasShadow
            || hasOutline || hasText || hasPseudo;
    } catch {
        return false;
    }
}
...
/** Клики проходят сквозь этот слой. Целью для правки он быть может. */
...
function onKey(e) {
    ...
    // Ctrl отключает выбор декоративных слоёв: берётся ровно то,
    // во что попал бы обычный клик
