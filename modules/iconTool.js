        ...
        h('div.vte-icon-block', {}, [
            ...
            h('small.vte-note', {
                text: 'Силуэт — картинка перекрашивается в выбранный цвет. Это же значение попадёт в background-color. '
                    + 'Работает только с картинками, которые сайт разрешает брать для маски (CORS), '
                    + 'или с кодом SVG. С обычных хостингов картинок силуэт часто просто пропадает — '
                    + 'тогда выбирайте «Картинка».',
            }),
            els.fitRow,
            els.pxRow,
            els.repeatRow,
...
function fxSlider(label, key, min, max, unit) {
    ...
    els[`fx_${key}`] = { range, out, unit, row };
    return row;
}

function syncFxInputs() {
    // Лёгкий режим: размытие картинки не предлагаем. Если оно уже есть —
    // показываем, чтобы его можно было убрать
    const blurRow = els.fx_blur?.row;
    if (blurRow) blurRow.style.display = isLite() && !fx.blur ? 'none' : '';
...
function svgToUrl(svg) {
    // Мнемоника собирается из \u0026 ('&') и остатка строки. Писать её
    // литералом нельзя: в любом тексте, который проходит через HTML,
    // ''' превращается в сам апостроф, и строка перестаёт закрываться.
    const APOS = '\u0026#39;';
    ...
    let clean = String(svg)
        ...
        .replace(/<!--[\s\S]*?-->/g, '')
        .replace(/\s+/g, ' ')
        // Апострофы в тексте и в именах шрифтов рассыпали разметку:
        // двойные кавычки заменялись на одинарные, а имеющиеся одинарные
        // не экранировались
        .replace(/'/g, APOS)
        .replace(/"/g, "'")
        .trim();
...
function apply() {
    ...
    /* ---- Сам рисунок ----
       Пишем шорткатом: 'mask: url(...) center / contain no-repeat' покрывает
       -image, -position, -size и -repeat одной строкой вместо четырёх.
       Плюс обязательно чистим набор противоположного режима: раньше при
       переключении «Картинка» → «Силуэт» в CSS оставались висеть мёртвые
       background-repeat / position / size при background-image: none. */
    if (mode === 'mask') {
        const shorthand = `${url} ${pos} / ${size} ${repeat}`;
        decls['mask'] = shorthand;
        decls['-webkit-mask'] = shorthand;
        decls['background-color'] = tint;
        decls['background-image'] = 'none';

        decls['background-repeat'] = '';
        decls['background-position'] = '';
        decls['background-size'] = '';
        decls['background-blend-mode'] = '';
        decls['mask-image'] = '';
        decls['-webkit-mask-image'] = '';
        decls['mask-repeat'] = '';
        decls['-webkit-mask-repeat'] = '';
        decls['mask-position'] = '';
        decls['-webkit-mask-position'] = '';
        decls['mask-size'] = '';
        decls['-webkit-mask-size'] = '';
    } else {
        decls['background'] = `${url} ${pos} / ${size} ${repeat}`;
    ...
    /* ---- Каркас слоя ----
       min-width и min-height, равные width и height, ничего не держали:
       это была защита от чужого flex-shrink, поставленная вслепую. От сжатия
       спасает сам flex-shrink: 0, который тут и так есть. */
    let hostDecls = null;
    ...
    /* ---- Состояние наведения ----
       Раньше сюда уходила полная копия ВСЕХ свойств плюс осветлённый цвет.
       На светлом тоне осветление возвращало исходное значение, и в CSS
       появлялся двадцатистрочный блок :hover, не отличающийся ни одним
       свойством. Теперь пишем ровно то, что реально меняется. */
    let hoverDecls = null;
    if (mode === 'mask') {
        const shifted = hoverShift(tint);
        if (String(shifted).toLowerCase() !== String(toHex(tint) || tint).toLowerCase()) {
            hoverDecls = { 'background-color': shifted };
        }
    }
...
/**
 * Цвет для состояния наведения.
 *
 * Осветление в лоб было бессмысленным на светлых цветах: lighten('#ffffff')
 * возвращал тот же '#ffffff', и расширение писало правило :hover, полностью
 * совпадающее с основным. Теперь светлое притемняем, тёмное осветляем.
 */
function hoverShift(color) {
    ...
    const k = lum > 0.7 ? -0.18 : 0.22;

    const move = (c) => Math.max(0, Math.min(255, Math.round(
        k > 0 ? c + (255 - c) * k : c * (1 + k)
    )));
...
function clearIcon() {
    ...
    onApply(empty, {
        // Пустое hover-правило создавать не надо: снятие фона само уберёт
        // и его. Раньше здесь писался блок :hover из одних пустых значений.
        hoverDecls: null,
...
function ensureGlyphBar() {
    ...
    els.glyphImgBtn = h('button.vte-icon-stash-btn', {
        ...
        title: 'Превратить глиф в PNG-картинку в исходном цвете. '
            + 'Работает всегда, но растр — при сильном увеличении мылится',
        on: { click: () => useGlyphAsImage(false, 'png') },
    ...
    els.glyphMaskBtn = h('button.vte-icon-stash-btn', {
        ...
        on: { click: () => useGlyphAsImage(true, 'png') },
    }, [icon('fa-fill-drip'), h('span', { text: ' Как силуэт' })]);

    els.glyphSvgBtn = h('button.vte-icon-stash-btn', {
        type: 'button',
        title: 'Превратить глиф в SVG: остаётся резким при любом размере и '
            + 'весит в разы меньше. Нарисуется, только если шрифт иконок '
            + 'установлен в системе — обязательно проверьте предпросмотр',
        on: { click: () => useGlyphAsImage(false, 'svg') },
    }, [icon('fa-vector-square'), h('span', { text: ' Как SVG' })]);
    ...
    els.glyphBar = h('div.vte-icon-stash.vte-icon-glyph', { style: 'display:none' }, [
        els.glyphIc, els.glyphText,
        els.glyphImgBtn, els.glyphMaskBtn, els.glyphSvgBtn, els.glyphGoBtn,
    ]);
...
function renderGlyphBar() {
    ...
    if (glyph) {
        ...
        els.glyphSvgBtn.style.display = 'inline-flex';
    ...
    if (glyphElsewhere && onRequestLayer) {
        ...
        els.glyphSvgBtn.style.display = 'none';
...
/**
 * Шрифт реально доступен для отрисовки.
 *
 * document.fonts.load возвращает промис, который решается и когда шрифт
 * не найден. Раньше результат не проверялся: если Font Awesome не успел
 * загрузиться, canvas молча рисовал фолбэк-букву, и она уходила в тему
 * вместо иконки.
 */
async function ensureGlyphFont(g) {
    const spec = `${g.weight || 900} 160px ${g.family}`;
    try { await document.fonts.load(spec, g.char); } catch {}
    try { return document.fonts.check(spec, g.char); } catch { return true; }
}

/**
 * Проверка отрисовки: сравниваем ширину глифа с шириной той же строки
 * в заведомо другом шрифте. Совпали — значит рисуется не иконка,
 * а подстановка.
 */
function glyphRenders(g) {
    try {
        ...
        ctx.font = `${g.weight || 900} 100px ${g.family}`;
        const withFont = ctx.measureText(g.char).width;

        ctx.font = '100px monospace';
        const fallback = ctx.measureText(g.char).width;

        if (!withFont) return false;
...
/** Глиф → PNG. Надёжно, но растр: мылится при увеличении. */
function glyphToPng(g, px, color) {
...
/**
 * Глиф → SVG. Остаётся вектором и весит около двухсот байт вместо
 * нескольких килобайт base64.
 *
 * Важное ограничение: шрифт внутри SVG, вставленного через background-image
 * или mask-image, документом не подгружается — такой SVG изолирован и своих
 * @font-face не получает. Иконка нарисуется только если Font Awesome
 * установлен в системе. Поэтому вариант предлагается рядом с PNG,
 * а не вместо него.
 */
function glyphToSvg(g, color) {
    const cp = g.char.codePointAt(0).toString(16).toUpperCase();
    const family = String(g.family)
        .split(',')[0]
        .replace(/['"]/g, '')
        .trim() || 'Font Awesome 6 Free';

    const svg = `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'>`
        + `<text x='50' y='50' text-anchor='middle' dominant-baseline='central' `
        + `font-family='${family}' font-weight='${g.weight || 900}' `
        + `font-size='82' fill='${color || '#ffffff'}'>&#x${cp};</text></svg>`;

    return svgToUrl(svg);
}

/**
 * @param asMask  силуэт вместо картинки
 * @param format  'png' | 'svg'
 */
async function useGlyphAsImage(asMask, format = 'png') {
    if (!glyph) return;
    const g = glyph;

    els.glyphText.textContent = 'Загружаю шрифт иконки…';

    const loaded = await ensureGlyphFont(g);
    if (!loaded || !glyphRenders(g)) {
        els.glyphText.textContent =
            `Шрифт ${String(g.family).split(',')[0].replace(/['"]/g, '')} `
            + 'сейчас недоступен: вместо иконки получилась бы обычная буква. '
    ...
    els.source.value = format === 'svg' ? prettifySource(url) : url;
    dirty = true;
    detectKind();
    ...
    els.glyphText.textContent = format === 'svg'
        ? 'Глиф превращён в SVG — остаётся резким при любом размере. '
          + 'Нарисуется, только если шрифт иконок установлен в системе. '
          + 'Проверьте предпросмотр и нажмите «Применить».'
        : (asMask
            ? 'Глиф превращён в силуэт (PNG). Выберите цвет и нажмите «Применить».'
            : 'Глиф превращён в картинку (PNG). Нажмите «Применить».');
}
